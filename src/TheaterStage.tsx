...
import { CanvasErrorBoundary } from './shell/canvas/CanvasErrorBoundary';
import { ProjectionBoothModal } from './shell/media/ProjectionBoothModal';
...
export function cleanKickChannel(raw?: string | null): string {
  if (!raw) return '';
  return raw
    .trim()
    .replace(/^kick:/i, '')
    .replace(/^https?:\/\/(?:www\.)?(?:player\.)?kick\.com\/(?:embed\/|video\/)?/i, '')
    .replace(/^@/, '')
    .replace(/^\/+|\/+$/g, '')
    .split('?')[0]
    .split('/')[0]
...
export function streamEmbedUrl(stream: StreamListing): string {
  const currentHost = typeof window !== 'undefined' ? window.location.hostname : 'movie-hell.pages.dev';

  if (stream.platform === 'kick') {
    const channel = cleanKickChannel(
      stream.channel || (stream.watchUrl ? stream.watchUrl.replace(/^https?:\/\/(?:www\.)?kick\.com\//i, '') : '') || stream.id
    );
    if (!channel) return stream.watchUrl || '';
    return `https://player.kick.com/${encodeURIComponent(channel)}?autoplay=true&muted=false`;
  }

  if ((stream.platform as string) === 'twitch' || stream.watchUrl?.includes('twitch.tv')) {
    const channel = (stream.channel || '').replace(/^twitch:/i, '').replace(/^https?:\/\/(?:www\.)?twitch\.tv\//i, '').replace(/^@/, '');
    return `https://player.twitch.tv/?channel=${encodeURIComponent(channel)}&parent=${encodeURIComponent(currentHost)}&autoplay=true&muted=false`;
  }
  ...
  if (stream.id.startsWith('meshcast:') || stream.embedUrl?.includes('meshcast.io') || stream.watchUrl?.includes('meshcast.io')) {
    return stream.embedUrl || stream.watchUrl || '';
  }

  if (stream.platform === 'mediamtx' || stream.id.startsWith('mediamtx:')) {
    return stream.embedUrl || stream.watchUrl || '';
  }

  if (stream.platform === 'cloudflare' || stream.id.startsWith('cloudflare:')) {
    return stream.embedUrl || stream.watchUrl || '';
  }

  if (stream.platform === 'vdo-ninja' || (stream.platform as string) === 'vdoninja' || stream.watchUrl?.includes('vdo.ninja')) {
    const rawUrl = stream.embedUrl || stream.watchUrl || '';
    ...
    return `https://vdo.ninja/?view=${encodeURIComponent(token)}&autoplay=1&cleanoutput=1&transparent=1&autostart=1&nocontrols=1&hideheader=1&nodirectorvideo=1`;
  }

  if (stream.platform === 'mock') {
    const title = encodeURIComponent(stream.name || 'Movie Hell Test Stream');
    return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(
      `<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720" viewBox="0 0 1280 720"><rect width="1280" height="720" fill="#121216"/><rect x="0" y="0" width="182" height="480" fill="#c0c0c0"/><rect x="182" y="0" width="182" height="480" fill="#c0c000"/><rect x="364" y="0" width="182" height="480" fill="#00c0c0"/><rect x="546" y="0" width="182" height="480" fill="#00c000"/><rect x="728" y="0" width="182" height="480" fill="#c000c0"/><rect x="910" y="0" width="182" height="480" fill="#c00000"/><rect x="1092" y="0" width="188" height="480" fill="#0000c0"/><rect x="0" y="480" width="1280" height="240" fill="#090a0f"/><text x="640" y="580" fill="#ffffff" font-family="system-ui, sans-serif" font-size="32" font-weight="700" text-anchor="middle">🎬 ${decodeURIComponent(title)}</text><text x="640" y="630" fill="#ffb703" font-family="monospace" font-size="20" text-anchor="middle">UNIFLORA RESEARCH SHELL — RECOGNIZE AUTONOMY</text></svg>`
    )}`;
  }
...
export default function TheaterStage({
  ...
  const [backstageMode, setBackstageMode] = useState<'none' | 'shrine' | 'khm_bruegel'>('none');
  const [isBoothOpen, setIsBoothOpen] = useState(false);

  // If not admin, ensure backstageMode stays 'none'
  useEffect(() => {
    if (!isAdmin && backstageMode !== 'none') {
      setBackstageMode('none');
    }
  }, [isAdmin, backstageMode]);
  ...
  useEffect(() => {
    const onFullscreenChange = () => {
      ...
      if (fs) {
        ...
        hudTimeoutRef.current = window.setTimeout(() => setShowFullscreenHud(false), 2200);
      } else {
  ...
  useEffect(() => {
    ...
    const onGlobalMouseMove = (e: MouseEvent | PointerEvent) => {
      if (hudTimeoutRef.current) window.clearTimeout(hudTimeoutRef.current);

      // If mouse is near the top edge (within 90px), show HUD
      if (e.clientY <= 90) {
        setShowFullscreenHud(true);
        ...
        // When mouse moves in the lower/middle screen, fade HUD to 0% opacity
        hudTimeoutRef.current = window.setTimeout(() => {
          setShowFullscreenHud(false);
        }, 1000);
      }
  ...
  const handleStageMouseMove = (e?: React.MouseEvent | React.PointerEvent) => {
    if (!isFullscreen) return;
    if (hudTimeoutRef.current) window.clearTimeout(hudTimeoutRef.current);

    if (e && e.clientY <= 90) {
      setShowFullscreenHud(true);
      hudTimeoutRef.current = window.setTimeout(() => setShowFullscreenHud(false), 2500);
    } else {
      hudTimeoutRef.current = window.setTimeout(() => setShowFullscreenHud(false), 1000);
    }
  };
  ...
  const [traceColor, setTraceColor] = useState<string>(() => {
  ...
  useEffect(() => {
    if (universalColor && universalColor !== traceColor) {
      setTraceColor(universalColor);
    }
  }, [universalColor]);

  // Synchronize universal color across windows and canvas docks
  useEffect(() => {
    ...
    bc.onmessage = (event) => {
      ...
      if (data.type === 'sync_universal_color' && typeof data.color === 'string') {
        setTraceColor(data.color);
      }
    };
    return () => bc.close();
  }, []);

  const selectTraceColor = (newColor: string) => {
    let formatted = newColor.trim();
    if (!formatted.startsWith('#') && /^[0-9a-f]{6}$/i.test(formatted)) {
      formatted = '#' + formatted;
    }
    setTraceColor(formatted);
  ...
  const [traceAnalogousSwatches, setTraceAnalogousSwatches] = useState<string[]>(() => generateAnalogousPalette(traceColor));

  useEffect(() => {
    setTraceAnalogousSwatches(generateAnalogousPalette(traceColor));
  }, [traceColor]);

  const handlePickTraceMovieColor = async () => {
    const picked = await pickScreenColor();
    if (picked) {
      selectTraceColor(picked);
    }
  };
  ...
  const handleExportTraceComposite = async () => {
    const traceCanvas = traceCanvasRef.current;
    if (!traceCanvas) return;

    const w = traceCanvas.width || 1920;
    const h = traceCanvas.height || 1080;
    ...
    if (!capturedVideo && navigator.mediaDevices?.getDisplayMedia) {
      try {
        ...
        // User cancelled or browser declined
      }
    }
    ...
    ctx.save();
    ctx.globalAlpha = Math.max(0.05, Math.min(1, traceOpacity));
    ctx.drawImage(traceCanvas, 0, 0, w, h);
    ctx.restore();

    try {
      const link = document.createElement('a');
      link.download = `cinema-trace-${roomName.toLowerCase().replace(/\s+/g, '-')}-reel${canvasActivePage}-composite.png`;
      link.href = exportCanvas.toDataURL('image/png');
      link.click();
    } catch (err) {
      console.warn('Trace composite export error:', err);
    }
  };

  const handleExportTraceTransparent = () => {
    const traceCanvas = traceCanvasRef.current;
    if (!traceCanvas) return;
    ...
    try {
      const link = document.createElement('a');
      link.download = `cinema-trace-${roomName.toLowerCase().replace(/\s+/g, '-')}-reel${canvasActivePage}-overlay.png`;
      link.href = exportCanvas.toDataURL('image/png');
      link.click();
    } catch (err) {
      console.warn('Trace transparent export error:', err);
    }
  };

  const handleDedicateTraceToShrine = () => {
    const traceCanvas = traceCanvasRef.current;
    if (!traceCanvas) return;
    try {
      ...
      console.warn('Trace dedicate error:', err);
    }
  };
const isKickStream =
  ...
  const effectiveHlsUrl = activeStream?.hlsUrl || (
    isOwncastStream && activeStream?.watchUrl
      ...
      : activeStream?.watchUrl?.includes('.m3u8')
  ...
  }, [activeStream, effectiveHlsUrl, backstageMode, reloadKey]);

  const handleReturnToMovieStage = () => {
    setBackstageMode('none');
  ...
  useEffect(() => {
    if (activeStream && activeStream.status !== 'offline') {
      ...
      setStreamLoaded(false);
      setCurtainState('closed');
      setAnnouncement('Curtains drawn closed.');
    }
  }, [activeStream?.id, activeStream?.status, reloadKey]);
    ...
    >
      ...
      <div className="theater-marquee-header">
        ...
        <div className="theater-controls">
          ...
          <button
            type="button"
            className="theater-btn theater-btn-dropper"
            onClick={handlePickTraceMovieColor}
            title="Sample live color directly from movie frame with Eyedropper"
            aria-label="Sample live movie color"
            style={{ borderColor: 'rgba(216, 182, 107, 0.4)', color: '#f3d899' }}
          >
            💧 Dropper
          </button>
          ...
          {isAdmin && (
            <>
              <button
                type="button"
                className={`theater-btn theater-btn-khm ${backstageMode === 'khm_bruegel' ? 'active' : ''}`}
                onClick={() => setBackstageMode((prev) => (prev === 'khm_bruegel' ? 'none' : 'khm_bruegel'))}
                title={backstageMode === 'khm_bruegel' ? 'Return to cinema screening' : 'Walk through the 3D KHM Vienna Bruegel Museum (as in Museum Hours)'}
                aria-pressed={backstageMode === 'khm_bruegel'}
                style={{ borderColor: '#ffd700', color: '#fff', background: backstageMode === 'khm_bruegel' ? '#7c1a2d' : undefined }}
              >
                {backstageMode === 'khm_bruegel' ? '🎬 View Screen' : '🏰 KHM 3D Tour'}
              </button>

              <button
                type="button"
                className={`theater-btn theater-btn-shrine ${backstageMode === 'shrine' ? 'active' : ''}`}
                onClick={() => setBackstageMode((prev) => (prev === 'shrine' ? 'none' : 'shrine'))}
                title={backstageMode === 'shrine' ? 'Return to cinema screening' : 'Peek behind the cinema screen to view the 3D rotating Art Shrine Pavilion'}
                aria-pressed={backstageMode === 'shrine'}
                style={{ borderColor: '#d8b66b', color: '#f3d899' }}
              >
                {backstageMode === 'shrine' ? '🎬 View Screen' : '⛩️ Behind Screen'}
              </button>
            </>
          )}
          ...
          <button
            type="button"
            className="theater-btn theater-btn-booth"
            onClick={() => setIsBoothOpen(true)}
            title="Open Projection Booth to broadcast or change live feed"
            aria-label="Open Projection Booth"
          >
            📽️ Projection Booth
          </button>
      ...
      <div className="proscenium-stage">
        ...
        {/* Screen / Stage Area */}
        <div className={`cinema-screen-container ${isAdmin && backstageMode !== 'none' ? 'backstage-revealed' : ''}`}>
          {isAdmin && backstageMode === 'khm_bruegel' ? (
            <div className="cinema-backstage-sanctuary">
              <KhmBruegelWalkthrough
                roomName={roomName}
                currentUserNickname="Cinema Master"
                onReturnToMovie={handleReturnToMovieStage}
                isBackstageView={true}
                isAdmin={isAdmin}
              />
            </div>
          ) : isAdmin && backstageMode === 'shrine' ? (
            <div className="cinema-backstage-sanctuary">
              <ShrinePavilion
                roomName={roomName}
                isBackstageView={true}
          ...
          {activeStream && (effectiveHlsUrl || embedUrl) ? (
            <div className="cinema-screen" style={{ display: isAdmin && backstageMode !== 'none' ? 'none' : 'block' }}>
              {effectiveHlsUrl ? (
                <video
                  ref={videoRef}
                ...
                <iframe
                  ...
                  allow="autoplay; camera; microphone; fullscreen; display-capture; picture-in-picture; encrypted-media; clipboard-write; web-share"
                  allowFullScreen
              ...
              {isTraceMode && (
                <CanvasErrorBoundary fallbackMessage="Screen trace canvas paused">
                  <div
      ...
      {isTraceMode && (
        <div className="theater-trace-toolbar" role="toolbar" aria-label="Cinema screen trace controls">
          ...
          <div className="trace-toolbar-bottom-row">
            ...
            {/* Color Swatches & Movie Eyedropper */}
            <div className="trace-color-palette" role="radiogroup" aria-label="Ink color palette">
              <button
                type="button"
                className="trace-swatch trace-eyedropper-btn"
                onClick={handlePickTraceMovieColor}
                title="Sample live movie color anywhere on screen with Eyedropper"
                aria-label="Sample live movie color"
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
              ...
              {CANVAS_PRESET_COLORS.map((c) => (
                <button
                  key={c.value}
                  type="button"
                  className={`trace-swatch ${traceColor.toLowerCase() === c.value.toLowerCase() ? 'selected' : ''}`}
                  style={{ backgroundColor: c.value, background: c.value, backgroundImage: 'none' }}
                  onClick={() => selectTraceColor(c.value)}
                  title={c.label}
                  aria-label={c.label}
                />
              ))}

              {/* Analogous Movie Blends */}
              {traceAnalogousSwatches.slice(0, 5).map((c, i) => (
                <button
                  key={`trace-analogous-${c}-${i}`}
                  type="button"
                  className={`trace-swatch trace-analogous-swatch ${traceColor.toLowerCase() === c.toLowerCase() ? 'selected' : ''}`}
                  style={{ backgroundColor: c, background: c, backgroundImage: 'none', borderColor: '#fcd34d' }}
                  onClick={() => selectTraceColor(c)}
                  title={`Analogous Blend: ${c}`}
                  aria-label={`Analogous Blend ${c}`}
                />
              ))}
              ...
              <input
                ...
                value={traceColor.startsWith('#') && traceColor.length === 7 ? traceColor : '#f3d899'}
                onInput={(e) => selectTraceColor(e.currentTarget.value)}
                onChange={(e) => selectTraceColor(e.currentTarget.value)}
            ...
            <div className="trace-export-group">
              <button
                type="button"
                className="trace-export-btn"
                onClick={handleExportTraceComposite}
                title="Capture current video frame layered underneath your trace drawing"
              >
                📸 Frame + Trace
              </button>
              <button
                type="button"
                className="trace-export-btn"
                onClick={handleExportTraceTransparent}
                title="Export drawing as a transparent PNG overlay at current opacity"
              >
                🏁 PNG Overlay
              </button>
              {isAdmin && onDedicateToShrine && (
                <button
                  type="button"
                  className="trace-export-btn trace-dedicate-btn"
                  onClick={handleDedicateTraceToShrine}
                  title="Dedicate your trace artwork as a wooden Ema tablet on the 3D Japanese Shrine"
        ...
        >
          <div className="fullscreen-hud-content">
            ...
            <div className="fullscreen-hud-controls">
              ...
              {/* Dedicate to Shrine */}
              {isAdmin && isTraceMode && onDedicateToShrine && (
                <button
                  type="button"
                  className="hud-btn"
                  onClick={handleDedicateTraceToShrine}
                  title="Dedicate your trace artwork as a wooden Ema tablet to the Shrine"
              ...
              {/* Backstage KHM 3D Tour & Shrine View */}
              {isAdmin && (
                <>
                  <button
                    type="button"
                    className={`hud-btn ${backstageMode === 'khm_bruegel' ? 'active' : ''}`}
                    onClick={() => setBackstageMode((prev) => (prev === 'khm_bruegel' ? 'none' : 'khm_bruegel'))}
                    title={backstageMode === 'khm_bruegel' ? 'Return to cinema screening' : 'Walk through the 3D KHM Vienna Bruegel Museum (as in Museum Hours)'}
                  >
                    {backstageMode === 'khm_bruegel' ? '🎬 Screen' : '🏰 KHM 3D'}
                  </button>

                  <button
                    type="button"
                    className={`hud-btn ${backstageMode === 'shrine' ? 'active' : ''}`}
                    onClick={() => setBackstageMode((prev) => (prev === 'shrine' ? 'none' : 'shrine'))}
                    title={backstageMode === 'shrine' ? 'Return to cinema screening' : 'Peek behind the cinema screen to view the 3D rotating Art Shrine Pavilion'}
                  >
                    {backstageMode === 'shrine' ? '🎬 Screen' : '⛩️ Shrine'}
                  </button>
                </>
              )}
