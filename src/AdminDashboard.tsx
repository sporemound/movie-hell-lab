...
import {
  ...
  StreamsResponse,
  TemporaryPass,
  TemporaryPassesResponse,
  CreateTemporaryPassResponse,
  TrustTier,
  User,
} from './types';
import { parseChannelAddress } from './utils/channelParser';

type AdminTab = 'rooms' | 'pitches' | 'users' | 'emotes' | 'audit' | 'provenance' | 'passes';
...
export function AdminDashboard({ user, token, onNavigateHome, onNavigateMod }: AdminDashboardProps) {
  ...
  const [autoFillStatus, setAutoFillStatus] = useState<{ type: 'success' | 'error' | 'info'; message: string } | null>(null);
  ...
  // Temporary Passes State
  const [passes, setPasses] = useState<TemporaryPass[]>([]);
  const [passesLoading, setPassesLoading] = useState(false);
  const [passLabel, setPassLabel] = useState('');
  const [passCustomCode, setPassCustomCode] = useState('');
  const [passDurationPreset, setPassDurationPreset] = useState('120');
  const [passCustomMinutes, setPassCustomMinutes] = useState(120);
  const [passRoomId, setPassRoomId] = useState('');
  const [passMaxUses, setPassMaxUses] = useState('');
  const [passBusy, setPassBusy] = useState(false);
  const [newlyCreatedPass, setNewlyCreatedPass] = useState<{ pass: TemporaryPass; password: string } | null>(null);
  ...
  const fetchProvenanceSources = async () => {
    ...
    try {
      const res = await fetch('/api/provenance/sources', { headers: authHeaders });
  ...
  const fetchPasses = async () => {
    ...
    try {
      const res = await fetch('/api/admin/temporary-passwords', { headers: authHeaders });
      const data = (await res.json()) as TemporaryPassesResponse;
      if (data.ok && Array.isArray(data.passes)) {
        setPasses(data.passes);
      }
    } catch {
      setError('Could not load temporary session passes.');
    } finally {
  ...
  const generateRandomPasscode = () => {
    const words = ['stream', 'cine', 'pass', 'lobby', 'reel', 'vault', 'screen', 'stage', 'premiere', 'guest'];
    const w1 = words[Math.floor(Math.random() * words.length)];
    const w2 = words[Math.floor(Math.random() * words.length)];
    const num = Math.floor(1000 + Math.random() * 9000);
    setPassCustomCode(`${w1}-${w2}-${num}`);
  };

  const handleCreatePass = async (e: FormEvent) => {
    e.preventDefault();
    if (!passLabel.trim()) return;
    setPassBusy(true);
    setError('');
    setNotice('');
    const durationMinutes = passDurationPreset === 'custom' ? Number(passCustomMinutes) : Number(passDurationPreset);
    const maxUses = passMaxUses.trim() ? Number(passMaxUses) : undefined;
    try {
      const res = await fetch('/api/admin/temporary-passwords', {
        method: 'POST',
        headers: authHeaders,
        body: JSON.stringify({
          label: passLabel.trim(),
          password: passCustomCode.trim() || undefined,
          durationMinutes: Number.isFinite(durationMinutes) && durationMinutes > 0 ? durationMinutes : 120,
          roomId: passRoomId.trim() || undefined,
          maxUses: maxUses && Number.isFinite(maxUses) && maxUses > 0 ? maxUses : undefined,
        }),
      });
      const data = (await res.json()) as CreateTemporaryPassResponse;
      if (!res.ok || !data.ok || !data.pass || !data.password) {
        throw new Error(data.error || 'Failed to create temporary session pass.');
      }
      setNewlyCreatedPass({ pass: data.pass, password: data.password });
      setNotice(`Temporary session pass "${passLabel}" created successfully!`);
      setPassLabel('');
      setPassCustomCode('');
      setPassMaxUses('');
      fetchPasses();
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Pass generation failed.');
    } finally {
      setPassBusy(false);
    }
  };
  ...
  const handleRevokePass = async (passId: string) => {
    if (!window.confirm('Are you sure you want to revoke this session pass immediately? Active sessions will no longer be grantable.')) {
      return;
    }
    setError('');
    setNotice('');
    try {
      const res = await fetch(`/api/admin/temporary-passwords/${encodeURIComponent(passId)}/revoke`, {
        method: 'POST',
        headers: authHeaders,
      });
      const data = (await res.json()) as { ok: boolean; error?: string };
      if (!res.ok || !data.ok) throw new Error(data.error || 'Failed to revoke session pass.');
      setNotice('Session pass revoked successfully.');
      fetchPasses();
    } catch (err: unknown) {
  ...
  const copyToClipboard = async (text: string, id: string, type: 'pass' | 'invite') => {
    try {
  ...
  const handleToggleMycotroph = async (targetUser: AdminUser) => {
    try {
      const res = await fetch(`/api/admin/users/${targetUser.id}/toggle-mycotroph`, {
        method: 'POST',
        headers: authHeaders,
      });
      const data = (await res.json()) as { ok: boolean; isMycotroph?: boolean; error?: string };
      if (!res.ok || !data.ok) throw new Error(data.error || 'Failed to toggle role.');
      setNotice(`Updated role for ${targetUser.nickname}: ${data.isMycotroph ? '🍄 Mycotroph Granted' : 'Standard Member'}`);
      fetchUsers();
    } catch (err: unknown) {
  ...
  const handleAutoFill = (inputToParse?: string) => {
    const raw = (inputToParse !== undefined ? inputToParse : autoFillInput).trim();
    if (!raw) {
      setAutoFillStatus({ type: 'error', message: 'Please enter a channel address, stream URL, or handle.' });
      return;
    }
    const result = parseChannelAddress(raw, boundarizePlatform);
    if (!result) {
      setAutoFillStatus({
        type: 'error',
        message: `Could not recognize channel address: "${raw}". Please enter a valid URL, hostname, or slug.`,
      });
      return;
    }
    setBoundarizeSourceId(result.sourceId);
    setBoundarizeName(result.name);
    setBoundarizePlatform(result.platform);
    setBoundarizeChannel(result.channel);
    setBoundarizeGuild(result.guild);
    setBoundarizeTrustTier(result.trustTier);
    setBoundarizeDomain(result.originDomain);
    setBoundarizeWatchUrl(result.watchUrl);
    setBoundarizeEmbedUrl(result.embedUrl);
    setBoundarizeHlsUrl(result.hlsUrl);
    setBoundarizeTags(result.boundaryTags);
    setBoundarizeAttestation(result.attestationNotes);
    setAutoFillStatus({
      type: 'success',
      message: `✨ Auto-filled 12 catalog fields for ${result.platform.toUpperCase()}: "${result.name}" (${result.channel})`,
    });
  };
  ...
  const handleSelectSourceToEdit = (s: StreamListing) => {
    ...
    setAutoFillInput(s.watchUrl || s.id);
    ...
    try {
      const res = await fetch('/api/admin/provenance/boundarize', {
        ...
        body: JSON.stringify({
          ...
          channel: (boundarizePlatform === 'kick'
            ? boundarizeChannel.trim().replace(/^https?:\/\/(?:www\.)?(?:player\.)?kick\.com\//i, '').replace(/^@/, '').replace(/^\/+|\/+$/g, '').split('?')[0].split('/')[0]
            : boundarizeChannel.trim()) || undefined,
    ...
    <div className="dashboard-container admin-dashboard">
      ...
      <nav className="dashboard-tabs" role="tablist">
        ...
        <button
          type="button"
          role="tab"
          aria-selected={activeTab === 'passes'}
          className={`dash-tab ${activeTab === 'passes' ? 'active' : ''}`}
          onClick={() => setActiveTab('passes')}
        >
          🎟️ Session Passes ({passes.length})
        </button>
        <button
          type="button"
          role="tab"
      ...
      <main className="dashboard-content">
                            ...
                            <tr key={u.id}>
                              ...
                              <td>
                                <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap', alignItems: 'center' }}>
                                  ...
                                  {u.isMycotroph && (
                                    <span className="chatter-badge" style={{ background: 'rgba(181, 95, 230, 0.22)', borderColor: '#b55fe6', color: '#e9d5ff' }} title="Uniflora Discord Guild Verified">
                                      🍄 Mycotroph
                                    </span>
                                  )}
                                </div>
                              ...
                              <td>
                                <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                                  <button
                                    ...
                                    title={u.isMycotroph ? "Revoke Mycotroph role" : "Grant Mycotroph role"}
                                    onClick={() => handleToggleMycotroph(u)}
                                    style={{
                                      background: u.isMycotroph ? 'rgba(181, 95, 230, 0.25)' : 'rgba(255, 255, 255, 0.08)',
                                      borderColor: u.isMycotroph ? '#b55fe6' : 'rgba(255, 255, 255, 0.2)',
                                      color: u.isMycotroph ? '#e9d5ff' : '#e5e7eb',
                                    }}
                                  >
                                    {u.isMycotroph ? '🍄 Revoke Mycotroph' : '🍄 Grant Mycotroph'}
                                  </button>
                                  <button
                                    type="button"
                                    className="dash-danger-btn small"
            ...
            <div className="dash-card">
              ...
              <div className="autofill-container">
                <div className="autofill-header">
                  <h4 className="autofill-title">
                    <span>🪄</span> Auto-Fill from Channel Address
                  </h4>
                  <span className="autofill-hint">
                    Enter any Kick URL, Picarto stream, Owncast host, or channel handle
                  </span>
                </div>
                <div className="autofill-input-row">
                  <input
                    id="catalog-autofill-input"
                    className="autofill-input"
                    type="text"
                    placeholder="e.g. https://kick.com/example_gamer, picarto.tv/example_artist, or stream.example.org"
                    value={autoFillInput}
                    onChange={(e) => {
                  ...
                  <button
                    ...
                    title="Parse address and populate all catalog fields"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}
                  >
                    <span>⚡</span> Auto-Fill Fields
                  </button>
                </div>
                <div className="autofill-samples">
                  <span>Quick Presets:</span>
                  <button
                    type="button"
                    className="autofill-chip"
                    onClick={() => {
                      setAutoFillInput('https://kick.com/example_gamer');
                      handleAutoFill('https://kick.com/example_gamer');
                    }}
                  >
                    Kick Sample
                  </button>
                  <button
                    ...
                    className="autofill-chip"
                    onClick={() => {
                      setAutoFillInput('https://picarto.tv/example_artist');
                      handleAutoFill('https://picarto.tv/example_artist');
                    }}
                  >
                    Picarto Sample
                  </button>
                  <button
                    ...
                    className="autofill-chip"
                    onClick={() => {
                      setAutoFillInput('https://stream.example.org');
                      handleAutoFill('https://stream.example.org');
                    }}
                  >
                    Owncast Sample
                  </button>
                ...
                {autoFillStatus && (
                  <div className={`autofill-alert ${autoFillStatus.type}`}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      {boundarizeSourceId && (
                        <button
                          type="button"
                          className="autofill-chip"
                          onClick={() => handleAutoFill(boundarizeSourceId)}
                          title="Auto-fill all fields based on this Source ID"
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const next = e.target.value as StreamPlatform;
                        setBoundarizePlatform(next);
                        if (!boundarizeDomain || boundarizeDomain === 'picarto.tv' || boundarizeDomain === 'stream.custom.org' || boundarizeDomain === 'kick.com') {
                          setBoundarizeDomain(next === 'kick' ? 'kick.com' : next === 'picarto' ? 'picarto.tv' : 'stream.custom.org');
                        }
                      }}
                    >
                      <option value="vdo-ninja">VDO.Ninja (P2P WebRTC / Decentralized)</option>
                      <option value="mock">Offline Test Pattern (Synthetic SMPTE)</option>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (boundarizePlatform === 'kick' && val.includes('kick.com')) {
                          val = val.replace(/^https?:\/\/(?:www\.)?(?:player\.)?kick\.com\//i, '').replace(/^@/, '').replace(/^\/+|\/+$/g, '').split('?')[0].split('/')[0];
                        }
                        setBoundarizeChannel(val);
                      }}
                    />
                  </div>
                </div>
        ...
        {activeTab === 'passes' && (
          <section className="dash-section" aria-label="Temporary Session Passwords">
            <div className="dash-two-col">
              <div className="dash-card">
                <h3>🎟️ Issue Temporary Session Pass</h3>
                <p className="card-desc">
                  Generate a short-term, semi-public passcode for a single livestream screening or guest audience.
                </p>

                {newlyCreatedPass && (
                  <div className="pass-created-callout">
                    <div className="pass-callout-header">
                      <strong>✨ Passcode Ready to Share</strong>
                      <button
                        type="button"
                        className="btn-sm btn-ghost"
                        onClick={() => setNewlyCreatedPass(null)}
                        aria-label="Dismiss banner"
                      >
                        ✕
                      </button>
                    </div>
                    <p className="pass-callout-label">{newlyCreatedPass.pass.label}</p>
                    <div className="pass-code-box">
                      <code className="pass-code-text">{newlyCreatedPass.password}</code>
                      <button
                        type="button"
                        className="btn-sm btn-primary"
                        onClick={() => copyToClipboard(newlyCreatedPass.password, newlyCreatedPass.pass.id, 'pass')}
                      >
                        {copiedPassId === newlyCreatedPass.pass.id ? '✅ Copied!' : '📋 Copy Passcode'}
                      </button>
                    </div>
                    <div className="pass-invite-box">
                      <input
                        type="text"
                        readOnly
                        value={`${window.location.origin}/?pass=${encodeURIComponent(newlyCreatedPass.password)}`}
                        className="pass-invite-input"
                      />
                      <button
                        type="button"
                        className="btn-sm btn-secondary"
                        onClick={() => copyToClipboard(`${window.location.origin}/?pass=${encodeURIComponent(newlyCreatedPass.password)}`, newlyCreatedPass.pass.id, 'invite')}
                      >
                        {copiedInviteId === newlyCreatedPass.pass.id ? '✅ Copied Link!' : '🔗 Copy Invite Link'}
                      </button>
                    </div>
                    <small className="pass-callout-note">
                      Expires in {newlyCreatedPass.pass.durationMinutes} minutes ({new Date(newlyCreatedPass.pass.expiresAt).toLocaleTimeString()}).
                    </small>
                  </div>
                )}

                <form onSubmit={handleCreatePass} className="dash-form">
                  <div className="form-group">
                    <label htmlFor="pass-label">Session Label / Title *</label>
                    <input
                      id="pass-label"
                      type="text"
                      placeholder="e.g. Friday Night Kick Screening, Premiere Guest Pass"
                      value={passLabel}
                  ...
                  <div className="form-grid">
                    <div className="form-group">
                      <label htmlFor="pass-duration">Validity Duration *</label>
                      <select
                        id="pass-duration"
                        value={passDurationPreset}
                        onChange={(e) => setPassDurationPreset(e.target.value)}
                      >
                        <option value="30">30 Minutes</option>
                        <option value="60">1 Hour</option>
                        <option value="120">2 Hours (Standard Screening)</option>
                        <option value="240">4 Hours (Double Feature)</option>
                        <option value="480">8 Hours (Marathon)</option>
                        <option value="1440">24 Hours (Full Day)</option>
                        <option value="custom">Custom Minutes...</option>
                      </select>
                    </div>

                    {passDurationPreset === 'custom' && (
                      <div className="form-group">
                        <label htmlFor="pass-custom-minutes">Custom Duration (Minutes)</label>
                        <input
                          id="pass-custom-minutes"
                          type="number"
                          min={1}
                          max={43200}
                          value={passCustomMinutes}
                          onChange={(e) => setPassCustomMinutes(Math.max(1, Number(e.target.value)))}
                        />
                      </div>
                    )}

                    <div className="form-group">
                      <label htmlFor="pass-room">Target Screening Room</label>
                      <select
                        id="pass-room"
                        value={passRoomId}
                        onChange={(e) => setPassRoomId(e.target.value)}
                      >
                        <option value="">All Rooms / Lobby Default</option>
                        {rooms.map((r) => (
                  ...
                  <div className="form-grid">
                    <div className="form-group">
                      <label htmlFor="pass-code">
                        Passcode Entry <small>(leave blank to auto-generate)</small>
                      </label>
                      <div className="input-with-button">
                        <input
                          id="pass-code"
                          type="text"
                          placeholder="e.g. stream-night-4820"
                          value={passCustomCode}
                          onChange={(e) => setPassCustomCode(e.target.value)}
                          minLength={6}
                          maxLength={128}
                        />
                        <button
                          ...
                          title="Generate readable random passcode"
                        >
                          🎲 Generate
                        </button>
                      </div>
                    ...
                    <div className="form-group">
                      <label htmlFor="pass-max-uses">
                        Max Redemptions <small>(leave blank for unlimited)</small>
                      </label>
                      <input
                        id="pass-max-uses"
                        type="number"
                        ...
                        value={passMaxUses}
                        onChange={(e) => setPassMaxUses(e.target.value)}
                      />
                    </div>
                  </div>

                  <button type="submit" className="btn-primary" disabled={passBusy}>
                    {passBusy ? 'Generating Pass...' : '✨ Generate Temporary Session Pass'}
                  </button>
                </form>
              </div>
              ...
              <div className="dash-card">
                <div className="dash-card-header">
                  <div>
                    <h3>📜 Active & Recent Session Passes</h3>
                    <p className="card-desc">
                      Review active passes, view live redemption counts, copy invite URLs, or revoke access.
                    </p>
                  </div>
                  <button type="button" className="btn-sm btn-secondary" onClick={fetchPasses} disabled={passesLoading}>
                    🔄 Refresh
                  </button>
                </div>

                {passesLoading ? (
                  <p className="loading-text">Loading session passes...</p>
                ) : passes.length === 0 ? (
                  <div className="empty-state">
                    <p>No temporary session passes have been created yet.</p>
                  </div>
                ) : (
                  <div className="dash-table-wrap">
                    <table className="dash-table">
                      ...
                      <tbody>
                        {passes.map((p) => {
                          ...
                          const isExhausted = typeof p.maxUses === 'number' && p.useCount >= p.maxUses;
                          const minutesRemaining = Math.max(0, Math.round((expTime - now) / 60000));
                          ...
                          const remainingText = isRevoked
                            ...
                            ? 'Full'
                            : hoursRemaining > 0
                            ? `${hoursRemaining}h ${minutesRemaining % 60}m left`
                            : `${minutesRemaining}m left`;

                          return (
                            <tr key={p.id} className={isRevoked || isExpired ? 'row-muted' : ''}>
                              <td>
                              ...
                              <td>
                                {p.roomName ? (
                                  ...
                                  <span className="table-subtext">All Rooms</span>
                                )}
                              </td>
                              <td>
                                <span>{p.useCount}</span>
                                <span className="table-subtext"> / {p.maxUses ? p.maxUses : '∞'}</span>
                              </td>
                              <td>
                                <div className="status-cell">
                                  ...
                                  >
                                    {isRevoked ? 'Revoked' : isExpired ? 'Expired' : isExhausted ? 'Exhausted' : 'Active'}
                                  </span>
                                  <span className="table-subtext">{remainingText}</span>
                                </div>
                              </td>
                              <td>
                                <span className="table-subtext">{p.creatorNickname || 'Admin'}</span>
                              </td>
                              <td>
                                <div className="table-actions">
                                  {!isRevoked && !isExpired && (
                                    <button
                                      ...
                                      title="Revoke this pass immediately"
                                    >
                                      🚫 Revoke
                                    </button>
                                  )}
