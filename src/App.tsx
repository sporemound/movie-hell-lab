...
import { ShrinePavilion } from './ShrinePavilion';
...
function StreamDirectory({
  ...
  const liveCount = listings.filter((stream) => stream.status !== 'offline').length + (activeStreamId && !listings.some((s) => s.id === activeStreamId) ? 1 : 0);
  ...
  return (
    <div id="room-canvas-panel" className="canvas-panel" role="tabpanel" aria-labelledby="room-canvas-tab">
      <KritaStudio
        ...
        onChangeUniversalColor={onChangeUniversalColor}
        onDedicateToShrine={onDedicateToShrine}
      />
    </div>
  );
}

function generateRandomCinemaHandle(): string {
...
export default function App() {
  ...
  const [discordGuildId] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      return params.get('guild') || (params.get('discord') ? '1282222320764715101' : '');
    }
    return '';
  });
  const [discordUserId] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      return params.get('discordUserId') || params.get('discordUser') || params.get('discord_user_id') || '';
    }
    return '';
  });
  const [mode, setMode] = useState<AuthMode>(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      if (params.get('pass') || params.get('guild') || params.get('discord') || params.get('discordUserId')) return 'pass';
    }
    return 'login';
  });
  ...
  const [nickname, setNickname] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      if (params.get('pass') || params.get('guild') || params.get('discord') || params.get('discordUserId')) {
        return generateRandomCinemaHandle();
      }
    }
    return '';
  });
  ...
  const [mobileLoungeTab, setMobileLoungeTab] = useState<'chat' | 'canvas' | 'rooms' | 'lineup'>('chat');
  ...
  useEffect(() => {
    if (SAFETY_LOCKDOWN || !accessToken) return;
  ...
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const roomParam = params.get('roomId') || params.get('room');
    if (roomParam) {
      setSelectedRoomId(roomParam);
    }
    const streamParam = params.get('streamId') || params.get('stream');
    if (streamParam) {
      ...
      if (match) {
        ...
        setActiveStream({
          ...
          channel: path,
          name: `MediaMTX (${path})`,
          description: 'Live MediaMTX Stream',
          watchUrl: targetUrl,
          embedUrl: targetUrl,
          status: 'live',
          viewers: 1,
          currentTitle: `Live Screening in Room ${roomParam || 'Auditorium'}`,
          mature: false,
        });
        ...
        setActiveStream({
          ...
          platform: 'vdo-ninja',
          channel: token,
          name: `VDO.Ninja (${token})`,
          description: 'Live VDO.Ninja Stream',
          watchUrl: targetUrl,
          embedUrl: targetUrl,
          status: 'live',
          viewers: 1,
  ...
  const [universalColor, setUniversalColor] = useState<string>(() => {
    try {
  ...
  const [shrineDraftArt, setShrineDraftArt] = useState<string | null>(null);

  // If not admin, ensure roomView cannot be set to 'shrine'
  useEffect(() => {
    if (roomView === 'shrine' && !user?.isAdmin) {
      setRoomView('chat');
    }
  }, [roomView, user?.isAdmin]);

  const handleDedicateCanvasToShrine = (dataUrl: string) => {
    setShrineDraftArt(dataUrl);
    if (user?.isAdmin) {
      setRoomView('shrine');
    }
  };
  ...
  const handleReturnToMovie = () => {
    setRoomView('chat');
    const video = document.querySelector('video.stage-embed-video') as HTMLVideoElement | null;
    if (video) {
      video.play().catch(() => {});
    }
    const stageEl = document.querySelector('.theater-stage-container') || document.querySelector('.proscenium-stage');
    if (stageEl) {
  ...
  // Inter-window canvas stroke, layer & color synchronization across popouts & main tabs
  useEffect(() => {
    ...
    bc.onmessage = (event) => {
      ...
      if (data.type === 'sync_universal_color' && typeof data.color === 'string') {
        setUniversalColor(data.color);
      } else if (data.roomId === selectedRoomId) {
  ...
  const authenticated = !SAFETY_LOCKDOWN && Boolean(accessToken && user);
  const selectedRoom = rooms.find((room) => roomId(room) === selectedRoomId) || (
    selectedRoomId
      ? {
          id: selectedRoomId,
          name: selectedRoomId.replace(/[-_]+/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()),
          description: 'Cinema Screening Room',
        }
      : undefined
  );
  ...
  useEffect(() => {
    if ((page !== 'home' && page !== 'popout-stage') || !accessToken) {
      setStreams([]);
      setActiveStream(null);
    ...
    setStreamsError('');
    api<StreamsResponse>('/api/streams', { signal: controller.signal }, accessToken)
      .then((result) => {
  ...
  }, [page, streamsRefreshKey, accessToken]);
  ...
  const submitAuth = async (event?: React.FormEvent<HTMLFormElement>) => {
    ...
    setNotice('');
    try {
      const handle = (nickname && nickname.trim()) ? nickname.trim() : generateRandomCinemaHandle();
      const body = {
        nickname: handle,
        guildId: discordGuildId || undefined,
        discordUserId: discordUserId || undefined,
      };
      const result = await api<AuthResponse & { targetRoomId?: string; passLabel?: string }>('/api/auth/quick-join', {
        method: 'POST',
      ...
      if (result.targetRoomId) {
        setSelectedRoomId(result.targetRoomId);
      }
      setNotice(`Welcome to Movie Hell! Entered as ${result.user.nickname || handle}.`);
    } catch (requestError) {
      setError(errorText(requestError, 'Unable to enter screening lounge.'));
    } finally {
      setPassword('');
      setBusy(false);
    }
  ...
  const handleRenameNickname = async () => {
    if (!user) return;
    const next = window.prompt('Enter your new chat display name / handle:', user.nickname);
    if (!next || !next.trim() || next.trim() === user.nickname) return;
    try {
      setBusy(true);
      ...
      if (!res.ok || !data.ok || !data.user) throw new Error(data.error || 'Failed to update handle.');
      setUser(data.user);
      try {
        localStorage.setItem('mh_user', JSON.stringify(data.user));
      } catch {}
      setNotice(`Display handle updated to "${data.user.nickname}".`);
    } catch (err: unknown) {
  ...
  const sendCanvasStroke = (stroke: CanvasStrokeDraft) => {
    const localStroke: CanvasStroke = {
      id: stroke.clientId || `stroke-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    ...
    // Universal interconnected canvas surfaces: instantly mark local canvas, trace screen, and pop-outs
    setCanvasStrokes((current) => orderedCanvasStrokes(current, [localStroke]));

    if (typeof BroadcastChannel !== 'undefined') {
      try {
        const bc = new BroadcastChannel('movie_hell_canvas_sync');
        bc.postMessage({ type: 'local_stroke', stroke: localStroke, roomId: selectedRoomId || 'default' });
        bc.close();
      } catch {}
    }
  ...
  const renderChatSection = (isDocked = false) => {
    const availableTabs: RoomView[] = isDocked
      ? ['chat', 'chatters', 'canvas']
      : user?.isAdmin
        ? ['chat', 'chatters', 'canvas', 'shrine', 'gallery']
        : ['chat', 'chatters', 'canvas', 'gallery'];

    const effectiveRoomView: RoomView = (isDocked && (roomView === 'shrine' || roomView === 'gallery'))
      ? 'chat'
      : (!user?.isAdmin && roomView === 'shrine')
        ? 'chat'
        : roomView;

    const handleTabKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>) => {
      const currentIndex = availableTabs.indexOf(effectiveRoomView);
      let next: RoomView | null = null;
      if (event.key === 'ArrowLeft') {
        next = availableTabs[(currentIndex - 1 + availableTabs.length) % availableTabs.length];
      } else if (event.key === 'ArrowRight') {
        next = availableTabs[(currentIndex + 1) % availableTabs.length];
      } else if (event.key === 'Home') {
        next = availableTabs[0];
      } else if (event.key === 'End') {
        next = availableTabs[availableTabs.length - 1];
      }
      if (!next) return;
      event.preventDefault();
      setRoomView(next);
      const tabId = isDocked ? `docked-room-${next}-tab` : `room-${next}-tab`;
    ...
    return (
      <section className={`panel messages ${isDocked ? 'theater-docked-chat' : ''}`} aria-labelledby="messages-heading">
        {selectedRoom ? (
          <>
            <div className="message-heading">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
                ...
                <button
                  type="button"
                  className={`theater-chatters-pill ${effectiveRoomView === 'chatters' ? 'active' : ''}`}
                  onClick={() => setRoomView(effectiveRoomView === 'chatters' ? 'chat' : 'chatters')}
            ...
            <div className="room-view-tabs" role="tablist" aria-label="Room feature">
              <button
                id={isDocked ? 'docked-room-chat-tab' : 'room-chat-tab'}
                type="button"
                role="tab"
                aria-selected={effectiveRoomView === 'chat'}
                aria-controls={isDocked ? 'docked-room-chat-panel' : 'room-chat-panel'}
                tabIndex={effectiveRoomView === 'chat' ? 0 : -1}
              ...
              <button
                id={isDocked ? 'docked-room-chatters-tab' : 'room-chatters-tab'}
                type="button"
                role="tab"
                aria-selected={effectiveRoomView === 'chatters'}
                aria-controls={isDocked ? 'docked-room-chatters-panel' : 'room-chatters-panel'}
                tabIndex={effectiveRoomView === 'chatters' ? 0 : -1}
              ...
              <button
                id={isDocked ? 'docked-room-canvas-tab' : 'room-canvas-tab'}
                type="button"
                role="tab"
                aria-selected={effectiveRoomView === 'canvas'}
                aria-controls={isDocked ? 'docked-room-canvas-panel' : 'room-canvas-panel'}
                tabIndex={effectiveRoomView === 'canvas' ? 0 : -1}
                onClick={() => setRoomView('canvas')}
                onKeyDown={handleTabKeyDown}
              >
                🎨 Shared canvas
              </button>
              {!isDocked && user?.isAdmin && (
                <button
                  id="room-shrine-tab"
                  type="button"
                  role="tab"
                  aria-selected={effectiveRoomView === 'shrine'}
                  aria-controls="room-shrine-panel"
                  tabIndex={effectiveRoomView === 'shrine' ? 0 : -1}
                  onClick={() => setRoomView('shrine')}
                  onKeyDown={handleTabKeyDown}
                >
                  ⛩️ Art Shrine
                </button>
              ...
              {!isDocked && (
                <button
                  ...
                  type="button"
                  role="tab"
                  aria-selected={effectiveRoomView === 'gallery'}
                  aria-controls="room-gallery-panel"
                  tabIndex={effectiveRoomView === 'gallery' ? 0 : -1}
            ...
            {effectiveRoomView === 'chat' ? (
              <div
                id={isDocked ? 'docked-room-chat-panel' : 'room-chat-panel'}
                className="room-view-panel"
                role="tabpanel"
                aria-labelledby={isDocked ? 'docked-room-chat-tab' : 'room-chat-tab'}
              >
              {messagesLoading ? (
          ...
          ) : effectiveRoomView === 'chatters' ? (
            <div
              id={isDocked ? 'docked-room-chatters-panel' : 'room-chatters-panel'}
              className="room-view-panel room-chatters-panel"
              role="tabpanel"
              aria-labelledby={isDocked ? 'docked-room-chatters-tab' : 'room-chatters-tab'}
            >
          ...
          ) : !isDocked && effectiveRoomView === 'gallery' ? (
            <div
              id="room-gallery-panel"
              className="room-view-panel room-gallery-panel"
              role="tabpanel"
              aria-labelledby="room-gallery-tab"
            >
              <TraditionalGallery
                roomName={selectedRoom.name}
                currentUserNickname={user?.nickname || 'Cinema Master'}
                onReturnToMovie={handleReturnToMovie}
                onDedicateToShrine={user?.isAdmin ? handleDedicateCanvasToShrine : undefined}
          ...
          ) : !isDocked && effectiveRoomView === 'shrine' && user?.isAdmin ? (
            <div
              id="room-shrine-panel"
              className="room-view-panel room-shrine-panel"
              role="tabpanel"
              aria-labelledby="room-shrine-tab"
            >
              <ShrinePavilion
                roomName={selectedRoom.name}
                currentUserNickname={user?.nickname || 'Cinema Artist'}
                draftArtworkUrl={shrineDraftArt}
                onClearDraftArtwork={() => setShrineDraftArt(null)}
                onReturnToMovie={handleReturnToMovie}
                isAdmin={Boolean(user?.isAdmin)}
                onShareToChat={(txt) => {
                  setRoomView('chat');
            ...
            <SharedCanvas
              ...
              universalColor={universalColor}
              onChangeUniversalColor={handleUpdateUniversalColor}
              onDedicateToShrine={user?.isAdmin ? handleDedicateCanvasToShrine : undefined}
            />
          )}
        </>
      ) : !authenticated ? (
        <div className="empty-screen auth-popout-card" style={{ padding: '1.75rem 1.25rem', textAlign: 'center' }}>
          <span aria-hidden="true" style={{ fontSize: '2.2rem' }}>🍿</span>
          <h2 id="messages-heading" style={{ marginTop: '0.35rem', fontSize: '1.15rem' }}>
            Enter Cinema Lounge
          </h2>
          <p style={{ fontSize: '0.8rem', color: '#9f9887', margin: '0.3rem 0 0.8rem 0' }}>
            Zero passwords required. Choose your handle to participate in live audience chat.
          </p>

          <form onSubmit={submitAuth} className="form-grid" style={{ width: '100%', maxWidth: '300px', margin: '0 auto', textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <label htmlFor="popout-nickname" style={{ margin: 0, fontSize: '0.78rem' }}>Cinema Handle</label>
              <button
                type="button"
                className="dash-action-btn small"
                style={{ padding: '0.1rem 0.4rem', fontSize: '0.68rem', minHeight: 0 }}
                onClick={() => setNickname(generateRandomCinemaHandle())}
                title="Generate a different cinema handle"
              >
                🎲 Re-roll
              </button>
            </div>
            <input
              ...
              spellCheck={false}
              maxLength={48}
              placeholder="e.g. CinemaGuest-4829"
              value={nickname}
              onChange={(e) => setNickname(e.target.value)}
            />
            <button type="submit" className="button-primary" style={{ marginTop: '0.6rem' }} disabled={busy}>
              {busy ? 'Connecting…' : `🍿 Enter as ${nickname || 'Guest'}`}
            </button>
      ...
      {page === 'popout-chat' ? (
        ...
        <div className="theater-popout-window">
          <TheaterStage
            ...
            roomId={selectedRoomId || 'auditorium'}
        ...
        <>
          <header className="cinema-topbar">
            ...
            <div className="topbar-right">
              {SAFETY_LOCKDOWN ? (
                <div className="user-badge-compact" style={{ borderColor: 'rgba(230, 57, 70, 0.75)', background: 'rgba(30, 8, 12, 0.85)', color: '#ff6b6b' }} aria-label="System status">
                  <span className="role-tag" style={{ background: '#e63946', color: '#fff', fontWeight: 800 }}>🔒 LOCKDOWN</span>
                  <span style={{ fontSize: '0.78rem', fontWeight: 600 }}>Accounts Suspended</span>
                </div>
              ) : authenticated ? (
                <div className="user-badge-compact" aria-label="Signed-in account">
                  <button
                    ...
                    title={user?.isAdmin ? 'Open Admin Control Room' : user?.isModerator ? 'Open Moderator Desk' : user?.isMycotroph ? 'Uniflora Discord Member' : 'Signed in as Member'}
                  >
                    {user?.isAdmin ? '👑 Admin' : user?.isModerator ? '🛡️ Mod' : user?.isMycotroph ? '🍄 Mycotroph' : '🎟️ Member'}
                  </button>
                  <strong className="user-nickname">{user?.nickname}</strong>
                  <button
                    type="button"
                    className="theater-popout-btn small"
                    style={{ padding: '0.15rem 0.4rem', fontSize: '0.72rem', minHeight: 0 }}
                    onClick={handleRenameNickname}
                    title="Change or finalize your chat handle"
                  >
                    ✏️
                  </button>
                ...
                <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                  <button
                    ...
                    style={{
                      background: 'rgba(216, 182, 107, 0.15)',
                      borderColor: 'rgba(216, 182, 107, 0.5)',
                      color: '#f3d899',
                    }}
                    onClick={() => {
                      setPage('home');
                      ...
                      setTimeout(() => {
                        ...
                        if (authStage) authStage.scrollIntoView({ behavior: 'smooth' });
                        const passInput = document.getElementById('session-passcode');
                        if (passInput) passInput.focus();
                      }, 100);
                    }}
                    title="Redeem a guest session pass"
                  >
                    🎟️ Session Pass
                  </button>
                  <button
            ...
            <>
              {authenticated ? (
                <>
                  {isChatDocked ? (
                    <>
                      <div className="mobile-lounge-switcher" role="tablist" aria-label="Mobile Lounge View">
                        <button
                          type="button"
                          role="tab"
                          aria-selected={mobileLoungeTab === 'chat'}
                          className={`mobile-tab-btn ${mobileLoungeTab === 'chat' ? 'active' : ''}`}
                          onClick={() => {
                            setMobileLoungeTab('chat');
                            setRoomView('chat');
                          }}
                        >
                          💬 Chat
                        ...
                        <button
                          type="button"
                          role="tab"
                          aria-selected={mobileLoungeTab === 'canvas'}
                          className={`mobile-tab-btn ${mobileLoungeTab === 'canvas' ? 'active' : ''}`}
                          onClick={() => {
                            setMobileLoungeTab('canvas');
                            setRoomView('canvas');
                          }}
                        >
                          🎨 Canvas
                        ...
                        <button
                          type="button"
                          role="tab"
                          aria-selected={mobileLoungeTab === 'rooms'}
                          className={`mobile-tab-btn ${mobileLoungeTab === 'rooms' ? 'active' : ''}`}
                          onClick={() => setMobileLoungeTab('rooms')}
                        >
                          🚪 Rooms
                        </button>
                        <button
                          type="button"
                          role="tab"
                          aria-selected={mobileLoungeTab === 'lineup'}
                          className={`mobile-tab-btn ${mobileLoungeTab === 'lineup' ? 'active' : ''}`}
                          onClick={() => setMobileLoungeTab('lineup')}
                        >
                          📺 Lineup
                        </button>
                      </div>

                      <div className={`cinema-lounge-grid mobile-view-${mobileLoungeTab}`}>
                        <TheaterStage
                          activeStream={activeStream}
                          allStreams={streams}
                          onSelectStream={handleWatchStream}
                  ...
                  <div className={`stream-directory-wrapper ${isChatDocked && mobileLoungeTab !== 'lineup' ? 'mobile-pane-hidden' : ''}`}>
                    <StreamDirectory
                      streams={streams}
              ...
              {SAFETY_LOCKDOWN ? (
                <section aria-labelledby="lockdown-heading" className="panel narrow auth-stage lockdown-stage" style={{ border: '2px solid rgba(230, 57, 70, 0.65)', background: 'linear-gradient(180deg, rgba(35, 10, 15, 0.95), rgba(18, 5, 8, 0.98))' }}>
                  <p className="panel-kicker" style={{ color: '#ff6b6b' }}>🚨 Emergency Security Status</p>
                  <h2 id="lockdown-heading" style={{ color: '#fff', fontSize: '1.4rem' }}>Complete Safety Lockdown</h2>
                  <p className="auth-intro" style={{ color: 'rgba(255, 255, 255, 0.88)', lineHeight: 1.6 }}>
                    All account activity, registrations, logins, temporary session pass redemptions, chat messaging, and screening streams have been <strong>temporarily suspended</strong> for safety and system maintenance.
                  </p>
                  <div style={{ marginTop: '1.25rem', padding: '1rem', borderRadius: '6px', background: 'rgba(0, 0, 0, 0.6)', border: '1px solid rgba(230, 57, 70, 0.3)', textAlign: 'center' }}>
                    <p style={{ margin: 0, fontSize: '0.86rem', color: '#ffcdd2', lineHeight: 1.5 }}>
                      🔒 No accounts or screenings are currently accessible. Please check back later.
                    </p>
                  </div>
                </section>
              ) : !authenticated ? (
                <section aria-labelledby="auth-heading" className="panel narrow auth-stage">
                  <p className="panel-kicker">Passwordless Screening Lounge</p>
                  <h2 id="auth-heading">Enter Movie Hell</h2>
                  <p className="auth-intro">
                    Zero passwords required. Choose your cinema handle or generate a new pseudonymous persona to join the audience, live chat, and atelier canvas.
                  </p>
                  <form onSubmit={submitAuth}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '0.9rem', marginBottom: '0.35rem' }}>
                      <label htmlFor="session-nickname" style={{ margin: 0, fontWeight: 600 }}>Your Cinema Handle</label>
                      <button
                        type="button"
                        className="dash-action-btn small"
                        style={{ padding: '0.2rem 0.55rem', fontSize: '0.76rem', minHeight: 0 }}
                        onClick={() => setNickname(generateRandomCinemaHandle())}
                        title="Generate a different random cinema handle"
                      >
                        🎲 Re-roll Handle
                      </button>
                    </div>
                    ...
                    />
                    <button type="submit" disabled={busy} className="button-primary" style={{ width: '100%', padding: '0.75rem 1rem', fontSize: '1rem', fontWeight: 600 }}>
                      {busy ? 'Connecting…' : `🍿 Enter Screening Lounge as ${nickname || 'Guest'}`}
                    </button>
                  </form>
                </section>
              ) : (
                <div className={`workspace ${isChatDocked ? 'workspace-solo-rooms' : ''} ${authenticated && isChatDocked && mobileLoungeTab !== 'rooms' ? 'mobile-pane-hidden' : ''}`}>
