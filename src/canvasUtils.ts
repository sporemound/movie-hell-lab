...
export const KRITA_BRUSH_PRESETS: BrushPresetInfo[] = [
  ...
  {
    id: 'sumi_brush',
    name: 'Sumi-e Bamboo Ink Brush',
    icon: '🖌️',
    category: 'paint',
    description: 'Dynamic calligraphic ink wash with wet bleeding and dry flying-white bristle streaks (feibai).',
    defaultWidth: 18,
    defaultOpacity: 0.9,
  },
  {
    id: 'woodcut_gouge',
    name: 'Mokuhanga Relief Gouge',
    icon: '🔪',
    category: 'sketch',
    description: 'V-gouge knife cuts carving crisp white relief grooves into woodblocks.',
    defaultWidth: 8,
    defaultOpacity: 1,
  },
  {
    ...
    icon: '🎨',
    category: 'paint',
    description: 'Crushed lapis, malachite, and cinnabar granular pigment with mica sheen.',
    defaultWidth: 16,
    defaultOpacity: 0.8,
  },
  {
    id: 'impasto_knife',
    name: 'Oil Impasto Palette Knife',
    icon: '🧈',
    category: 'paint',
    description: 'Thick raised sculptural oil paint strokes with specular edge highlights.',
    defaultWidth: 22,
    defaultOpacity: 0.95,
  },
  {
    id: 'gold_leaf',
    name: 'Gold Leaf & Sunago Gilding',
    icon: '🍂',
    category: 'fx',
    description: 'Shimmering polygon flakes of real gold leaf and sprinkled metallic gold dust.',
    defaultWidth: 20,
    defaultOpacity: 0.95,
  },
  {
    id: 'charcoal',
    name: 'Vine Charcoal & Stump',
    icon: '✏️',
    category: 'sketch',
    description: 'Velvety dark charcoal deposits with paper grain stippling and soft blending.',
    defaultWidth: 14,
    defaultOpacity: 0.85,
  },
  {
...
export function generateAnalogousPalette(hexColor: string): string[] {
  let formatted = hexColor.trim();
  if (!formatted.startsWith('#') && /^[0-9a-f]{6}$/i.test(formatted)) {
    formatted = '#' + formatted;
  }
  if (!/^#[0-9a-f]{6}$/i.test(formatted)) {
    return ['#f3d899', '#fcd34d', '#fbbf24', '#f59e0b', '#d97706'];
  }

  const rgb = hexToRgb(formatted);
  const hsv = rgbToHsv(rgb.r, rgb.g, rgb.b);
  const sRatio = Math.max(0.1, hsv.s / 100);
  const vRatio = Math.max(0.1, hsv.v / 100);
  ...
  for (const h of hues) {
    palette.push(hsvToHex(h, sRatio, vRatio));
  }

  palette.push(hsvToHex(hsv.h, Math.max(0.08, sRatio * 0.45), Math.min(1, vRatio * 1.25 + 0.15)));
  palette.push(hsvToHex(hsv.h, Math.min(1, sRatio * 1.25), Math.max(0.12, vRatio * 0.55)));
  palette.push(hsvToHex((hsv.h + 180) % 360, sRatio, vRatio));

  return Array.from(new Set(palette.map((c) => c.toLowerCase())));
}
...
function renderSingleStrokePath(
  ...
  switch (stroke.tool) {
    ...
    case 'sumi_brush': // Sumi-e Bamboo Ink Brush (pressure, bleed & feibai)
      renderSumiBrush(context, points, stroke.color, baseWidth, width, height);
      break;

    case 'woodcut_gouge': // Mokuhanga Woodcut Relief Gouge
      renderWoodcutGouge(context, points, baseWidth, width, height);
      break;

    case 'nihonga_mineral': // Nihonga Crushed Mineral Pigment
      renderNihongaMineral(context, points, stroke.color, baseWidth, width, height);
      break;

    case 'impasto_knife': // Oil Impasto Palette Knife
      renderImpastoKnife(context, points, stroke.color, baseWidth, width, height);
      break;

    case 'gold_leaf': // Gold Leaf & Sunago Gilding
      renderGoldLeaf(context, points, stroke.color, baseWidth, width, height);
      break;

    case 'charcoal': // Vine Charcoal & Blending Stump
      renderCharcoal(context, points, stroke.color, baseWidth, width, height);
      break;
...
/**
 * 🖌️ Sumi-e Bamboo Ink Brush: Pressure-sensitive wash, wet bleed pooling, and flying-white (feibai) dry streaks
 */
function renderSumiBrush(
  ctx: CanvasRenderingContext2D,
  points: CanvasPoint[],
  color: string,
  baseWidth: number,
  ...
  if (points.length < 2) {
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.arc(points[0].x * width, points[0].y * height, baseWidth * 0.7, 0, Math.PI * 2);
    ctx.fill();
    return;
  }

  // Multi-pass wet wash with soft feathered ink edge
  for (let i = 0; i < points.length - 1; i++) {
    const p0 = points[i];
    const p1 = points[i + 1];
    const x0 = p0.x * width;
    ...
    const pressure = p1.p !== undefined ? p1.p : 0.7;
    const segmentWidth = Math.max(2, baseWidth * (0.35 + pressure * 0.95));

    // Outer soft wet wash bleed
    ctx.strokeStyle = color;
    ctx.lineWidth = segmentWidth * 1.25;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ...
    // High speed dry bristle (feibai) split streaks
    const dist = Math.hypot(x1 - x0, y1 - y0);
    if (dist > 18) {
      ctx.lineWidth = Math.max(1, segmentWidth * 0.2);
      ctx.globalAlpha = 0.45;
      const angle = Math.atan2(y1 - y0, x1 - x0);
      const perpX = -Math.sin(angle) * (segmentWidth * 0.35);
      const perpY = Math.cos(angle) * (segmentWidth * 0.35);

      ctx.beginPath();
      ctx.moveTo(x0 + perpX, y0 + perpY);
      ctx.lineTo(x1 + perpX * 0.5, y1 + perpY * 0.5);
      ctx.stroke();
    }
  }
}
...
/**
 * 🔪 Mokuhanga Woodcut Gouge: Sharp relief chisel cutting clean carved grooves
 */
function renderWoodcutGouge(
  ctx: CanvasRenderingContext2D,
  points: CanvasPoint[],
  baseWidth: number,
  width: number,
  ...
  // Gouge cuts out existing pigment or creates crisp white relief
  ctx.save();
  ctx.strokeStyle = '#ffffff';
  ...
  // Subtle woodblock relief shadow edge
  ctx.strokeStyle = 'rgba(0, 0, 0, 0.4)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(points[0].x * width + 1, points[0].y * height + 1);
  for (let i = 1; i < points.length; i++) {
    ctx.lineTo(points[i].x * width + 1, points[i].y * height + 1);
  }
  ctx.stroke();
  ctx.restore();
}
...
/**
 * 🎨 Nihonga Mineral Pigment: Granular crushed stone texture with shimmering mica flakes
 */
function renderNihongaMineral(
  ctx: CanvasRenderingContext2D,
  points: CanvasPoint[],
  color: string,
  baseWidth: number,
  ...
  ctx.fillStyle = color;
  const radius = Math.max(2, baseWidth / 2);
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    // Granular pointillist mineral clusters
    for (let g = 0; g < 7; g++) {
      const offsetX = (Math.random() - 0.5) * radius * 1.8;
      const offsetY = (Math.random() - 0.5) * radius * 1.8;
      const grainSize = Math.max(1, (Math.random() * radius * 0.4));
    ...
    // Mica mineral glitter speckle
    if (Math.random() < 0.35) {
      ctx.fillStyle = '#fffdf0';
      ctx.globalAlpha = 0.8;
      ctx.fillRect(cx + (Math.random() - 0.5) * radius, cy + (Math.random() - 0.5) * radius, 1.5, 1.5);
      ctx.fillStyle = color;
    }
  }
}
...
/**
 * 🧈 Impasto Palette Knife: Sculpted oil paint with raised bevel edge and specular light
 */
function renderImpastoKnife(
  ctx: CanvasRenderingContext2D,
  points: CanvasPoint[],
  color: string,
  baseWidth: number,
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    // Raised specular highlight knife ridge
    ctx.strokeStyle = 'rgba(255, 255, 255, 0.45)';
    ctx.lineWidth = Math.max(1, baseWidth * 0.2);
    ctx.beginPath();
    ctx.moveTo(x0 - perpX * 0.8, y0 - perpY * 0.8);
    ctx.lineTo(x1 - perpX * 0.8, y1 - perpY * 0.8);
    ctx.stroke();

    // Dark under-bevel shadow
...
/**
 * 🍂 Gold Leaf & Sunago Gilding: Polygon flakes of hammered gold foil
 */
function renderGoldLeaf(
  ctx: CanvasRenderingContext2D,
  points: CanvasPoint[],
  color: string,
  baseWidth: number,
  ...
  const goldPalette = ['#ffd700', '#f59e0b', '#d97706', '#fef08a', '#eab308'];
  const radius = Math.max(3, baseWidth / 2);

  for (const pt of points) {
    const cx = pt.x * width;
    const cy = pt.y * height;

    // Irregular polygonal gold foil flake
    const flakeCount = Math.floor(Math.random() * 3) + 2;
    for (let f = 0; f < flakeCount; f++) {
      const goldColor = goldPalette[Math.floor(Math.random() * goldPalette.length)];
      ctx.fillStyle = goldColor;
      ctx.globalAlpha = 0.85 + Math.random() * 0.15;

      const fx = cx + (Math.random() - 0.5) * radius * 1.5;
      const fy = cy + (Math.random() - 0.5) * radius * 1.5;
      const fsize = radius * (0.4 + Math.random() * 0.7);

      ctx.beginPath();
      ctx.moveTo(fx, fy - fsize);
      ctx.lineTo(fx + fsize * 0.9, fy - fsize * 0.3);
      ctx.lineTo(fx + fsize * 0.6, fy + fsize * 0.8);
      ctx.lineTo(fx - fsize * 0.7, fy + fsize * 0.6);
      ctx.closePath();
      ctx.fill();
    }
  }
...
/**
 * ✏️ Vine Charcoal & Stump: Soft velvety charcoal deposits with paper grain stippling
 */
function renderCharcoal(
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    // Paper-tooth stipple dust
    for (let d = 0; d < 6; d++) {
      const offsetX = (Math.random() - 0.5) * radius * 1.6;
      const offsetY = (Math.random() - 0.5) * radius * 1.6;
      ctx.globalAlpha = 0.15 + Math.random() * 0.45;
      ctx.fillRect(cx + offsetX, cy + offsetY, 1.5, 1.5);
    }
  }
}
