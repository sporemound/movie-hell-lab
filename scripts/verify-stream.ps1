...
try {
    ...
    $permPass = $perm -like "*player.kick.com*"
    if ($cspPass -and $permPass) {
        Write-Host " [PASS]" -ForegroundColor Green
        Write-Host "      - CSP frame-src whitelist: Valid" -ForegroundColor Gray
        Write-Host "      - Permissions-Policy delegation: Valid" -ForegroundColor Gray
        $results["Security Headers"] = "PASS"
    } elseif ($cspPass) {
        Write-Host " [PASS: CSP Valid]" -ForegroundColor Green
        $results["Security Headers"] = "PASS (CSP only)"
    } else {
